...
class CommandParser:
    ...
    def parse_command(
            self, 
            input_command: str) -> Dict[str, Any]:
        """
        ...
        # The rest are options or positional arguments
        options = [word for word in words[1:] if word.startswith('--')]
        positional_args = [word for word in words[1:] if not word.startswith('--')]

        # If verbose is True, print the parsed arguments
        if self.verbose:
